...
export const actionableExtractionPrompt: PromptTemplate = {
  ...
  render: ({ sourceText, timezone, locale, language, currentDate }) =>
    [
      ...
      "- events: array of { title, description?, start?, end?, location?, recurrence?, confidence, source? }",
      ...
      "If the user mentions a recurring event, include recurrence as an array of RFC 5545 recurrence lines (e.g., RRULE:FREQ=WEEKLY;BYDAY=MO).",
